...
산업 표면 결함을 찾는 영상처리 파이프라인을 만들고, **CPU → CUDA → 멀티스레드** 순서로 단계별로 빠르게 만들어보면서 속도를 측정해 본 개인 학습 프로젝트입니다.

CUDA랑 실시간 처리를 공부하는 게 목적이라, 결함 검출은 딥러닝 대신 **고전 비전(차분 → 블러 → 이진화 → 모폴로지 → 라벨링)으로 구현**했습니다. MVTec AD 데이터셋을 카메라 스트림처럼 흘려보내서, 카메라 없이도 전체 파이프라인을 테스트합니다.
...
absdiff·가우시안 블러·이진화·모폴로지를 CUDA 커널로 작성했습니다. 라벨링(CCL)은 GPU 구현이 복잡해서 일단 CPU에 남겨뒀습니다.
...
**D2H 전송은 확실히 빨라졌는데, 정작 전체 속도(detect())는 거의 그대로였습니다.** 왜인지 봤더니 ① pageable 프레임을 pinned 버퍼로 옮기는 복사가 추가됐고, ② 전체 시간의 대부분은 여전히 CPU 라벨링이라 그렇더라고요. "교과서에 나온 최적화라고 무조건 전체가 빨라지는 건 아니다"를 확인한 단계였습니다. 전송을 비동기로 만들어 둔 덕은 다음 단계에서 봤습니다.
...
`프레임 공급 → [큐] → GPU → [큐] → CPU 라벨링` 구조로, 스테이지 사이를 **lock-free SPSC 큐**로 연결했습니다.
